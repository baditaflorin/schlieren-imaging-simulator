...
Scene presets are versioned TypeScript objects bundled with the app. Optional offline audio feature files may be generated as JSON by the `tools/librosa/` helper, but no such artifact is required for runtime. The app ships `build-info.json` as same-origin static metadata for the published version and build commit.
